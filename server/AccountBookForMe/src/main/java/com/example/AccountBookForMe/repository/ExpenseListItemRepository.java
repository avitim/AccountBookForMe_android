...
public interface ExpenseListItemRepository extends JpaRepository<ExpenseListItem, Long> {
    ...
    @Query(value = "SELECT e.id AS expense_id, '' AS purchased_at, e.purchased_at AS full_purchased_at, e.total_amount AS price, pm.name AS method, s.name AS store " +
            "FROM expenses e INNER JOIN stores s ON e.store_id = s.id " +
